...
class BusProblem(Problem):
    ...
    def _getNewStateAtLoc(self, previousState, newLoc):
        newOnBus = list(filter(lambda order: order[0] == newLoc, previousState.waitingOrders))
        #newOnBus now contains the orders that newLoc is the source of them

        newFinished = list(filter(lambda  order: order[1] == newLoc,previousState.ordersOnBus))
        #newFinished now contains the orders that newLoc is the target of them

        newWaiting = list(filter(lambda order: order not in newOnBus,previousState.waitingOrders))
        #newWaiting now contains the orders that still waiting

        newOnBus+= previousState.ordersOnBus
        newOnBus = list(filter(lambda order: order not in newFinished,newOnBus))
        #newOnBus now contains all the orders from the prev state plus the waiting orders that we picked up minus the
        #orders from the previous state that the target of them is newLoc

        newFinished = list(filter(lambda order: order not in newOnBus and order not in newWaiting,self.orders))
        #newFinished now contains all the finished orders
